// https://github.com/ggodone-maresca/JS-Animated-Bubbles-Background

/*
 * An object that stores different settings for the bubbles
 */
var bubbleOptions = {
  maxBubbles: 275, //A bubble ceiling, for high resolution monitors
  timer: -1, //The interval time
  tick: 125, //The tick speed
  ...
  light: 47, //The lightness
  lightRand: 7, //The lightness variance
  opacityFactor: 2.5, //What Math.random() opacity should be divided by
...
function instantiateBubbles() {
  ...
  var bubbleLength = Math.floor(
    window.innerWidth * window.innerHeight / bubbleOptions.ratio
  )
...
Bubble.prototype.create = function() {
  ...
  var hue =
    Math.floor(Math.random() * bubbleOptions.hueRand) + bubbleOptions.hue
  var saturation =
    Math.floor(Math.random() * bubbleOptions.saturationRand) +
    bubbleOptions.saturation
  var light =
    Math.floor(Math.random() * bubbleOptions.lightRand) + bubbleOptions.light
